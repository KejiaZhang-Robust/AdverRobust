...
if __name__ == '__main__':
    ...
    if config.Operation.Validate_Best == True:
        ...
        if config.Operation.Validate_Autoattack_L2:
            ##----->Autoattack
            auto_acc = evaluate_autoattack_l2(net, test_loader, config.ADV.clip_eps, auto_attacks_methods)
            logger.info(f"Auto_attack:[eps:{config.ADV.clip_eps}]->AA_acc: {auto_acc: .2f}")
    ...
    if config.Operation.Validate_Last == True:
        ...
        if config.Operation.Validate_Autoattack_L2:
            ##----->Autoattack
            auto_acc = evaluate_autoattack_l2(net, test_loader, config.ADV.clip_eps, auto_attacks_methods)
            logger.info(f"Auto_attack(L2):[eps:{config.ADV.clip_eps}]->AA_acc: {auto_acc: .2f}")
